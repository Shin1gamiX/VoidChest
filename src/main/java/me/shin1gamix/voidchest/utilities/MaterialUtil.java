...
public enum MaterialUtil {
	...
	MaterialUtil(int data, String... m) {
		this.m = m;
		this.data = data;
	}
	...
	public ItemStack parseItem() {
		...
		ItemStack item = isNewVersion() ? new ItemStack(mat) : new ItemStack(mat, 1, (byte) data);
		return item;
	}
	...
	private static Map<String, MaterialUtil> cachedSearch = new HashMap<>();

	public static MaterialUtil requestXMaterial(String name, byte data) {
		MaterialUtil cache = cachedSearch.get(name.toUpperCase() + "," + data);
		if (cache != null) {
			return cache;
		}
		for (MaterialUtil mat : MaterialUtil.values()) {
	...
	private static Map<Material, MaterialUtil> cachedMaterial = Maps.newHashMap();

	public static MaterialUtil fromMaterial(Material mat) {
		MaterialUtil mu = cachedMaterial.get(mat);
		if (mu != null) {
			return mu;
		}
		String matName = mat.toString().toUpperCase();
		try {
			return MaterialUtil.valueOf(matName);
		} catch (IllegalArgumentException e) {
			for (MaterialUtil xmat : MaterialUtil.values()) {
				for (String test : xmat.m) {
					if (test.equalsIgnoreCase(matName)) {
						cachedMaterial.put(mat, xmat);
						return xmat;
					}
				}
			}
	...
	private static Map<String, MaterialUtil> cachedString = new HashMap<>();

	public static Optional<MaterialUtil> fromString(String key) {
		if (key == null) {
			return Optional.empty();
		}

		key = key.toUpperCase();
		MaterialUtil mu = cachedString.get(key);
		if (mu != null) {
			return Optional.of(mu);
		}

		try {
			mu = MaterialUtil.valueOf(key);
		} catch (IllegalArgumentException e) {
			String[] split = key.split(":");
			mu = split.length == 1 ? requestXMaterial(key, (byte) 0)
					: requestXMaterial(split[0], (byte) Integer.parseInt(split[1]));
		}

		return mu != null ? Optional.of(mu) : Optional.empty();

	}

	private Boolean isDamageable = null;

	public boolean isDamageable() {
		if (this.isDamageable != null) {
	...
	private static Map<MaterialUtil, Material> cachedMaterialUtil = Maps.newHashMap();

	public Material parseMaterial() {

		Material lookup = null;

		for (String other : this.m) {
			lookup = Material.matchMaterial(other);
			if (lookup != null) {
				break;
			}
		}
		...
		if (lookup == null) {
			lookup = Material.matchMaterial(this.toString());
		}

		if (lookup == null) {
			lookup = Material.matchMaterial(this.m[0]);
		}

		if (lookup != null) {
			cachedMaterialUtil.put(this, lookup);
		}
